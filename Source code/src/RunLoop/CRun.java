        ...
        for (oil = 0; oil < rhOiList.length; oil++)
        {
            ...
            if (oilPtr.oilOi != 0x7FFF && (o & 0x8000) == 0)
            {
                ...
                if ((oiPtr.oiFlags & COI.OIF_GLOBAL) != 0)
                {
                    ...
                    // Rechercher l'objet dans les objets d���j��� cr������s
                    boolean flag = false;
...
/*	if (rhGameFlags&GAMEFLAGS_LOADONCALL)
         ...
         */
        // Efface les zones de memoire (pas si SLOW et QUICKDISPLAY)
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        boolean bToKill = false;
        if ((hoPtr.hoOEFlags & CObjectCommon.OEFLAG_SPRITES) == 0)			// Pas un sprite, on vire...
        {
            bToKill = true;
        }
        else
        {
            ...
            {
                if ((hoPtr.hoOEFlags & CObjectCommon.OEFLAG_QUICKDISPLAY) == 0)	// Un quickdisplay vire lors du screen_display
                {
                    if ((hoPtr.ros.rsCreaFlags & CSprite.SF_OWNERDRAW) == 0)
                    {
                        bToKill = true;                                         // Un VRAI sprite est detruit tout de suite
                    }                                                                                // Un ownerdraw dans le screen_display
                }
                else
                {
                  /*  // Vire un quickdisplay de la liste de display
                    if (hoPtr.hoNextQuickDisplay == -2 || hoPtr.hoNextQuickDisplay == -1)
                    {
                        hoPtr.hoNextQuickDisplay = -1;		// Force le redessin
                        display_OwnerQuickDisplay(hoPtr);
                    } */
                }
            }
        }
    ...
    {
        int			lgEdit, htEdit;
        int			i, obst, v = 0, cm_box = 0, x2edit, y2edit, flgColMaskEmpty = 0;
        short		img = 0;
        CLO		plo = null;
        CObject	hoPtr = null;
        ...
        // Build 248 : d����plac���� ici avant le F_UpdateWindowPos
        nLayer = 0;
        for (; nLayer < (int)rhFrame.nLayers; nLayer++)
        {
            CLayer pLayer = rhFrame.layers[nLayer];
        ...
        for ( ; nLayer < rhFrame.nLayers; nLayer++)
        {
          ...
          /*  if ( (flags & DLF_REDRAWLAYER) != 0 )
            ...
            // Display backdrop objects created at yourapplication
            if ( pLayer.pBkd2 != null )
                displayBkd2Layer(pLayer, nLayer, flags, x2edit, y2edit, flgColMaskEmpty != 0);
...
//	    yDest = 0;
        ...
        for (int nObjects = 0; nObjects < rhNObjects; nObjects++)
        {
          ...
          /*  if (pHo.hoNextQuickDisplay != -2)
            ...
            if (noMove != 0)
            {
                ...
                if ((pHo.hoOEFlags & CObjectCommon.OEFLAG_BACKGROUND) == 0)					// protection ajout���e build 96: crash si Q/A object + scrolling
                {
                    pHo.modif();
                }
            }
    ...
    public void activeToBackdrop(CObject hoPtr, int nTypeObst, boolean bTrueObject)
    {
        ...
        toadd.loHnd = 0;
        toadd.oiHnd = 0;
        toadd.x = hoPtr.hoX - ifo.getXSpot();
        toadd.y = hoPtr.hoY - ifo.getYSpot();
    ...
    public void addBackdrop2(CBkd2 toadd)
    {
        ...
        if (pLayer.pBkd2 != null)
        {
            for (i = 0; i < pLayer.pBkd2.size(); i++)
            {
                ...
                if (pbkd.x == toadd.x && pbkd.y == toadd.y && pbkd.nLayer == toadd.nLayer && pbkd.img == toadd.img && (pbkd.inkEffect & CSpriteGen.EFFECT_MASK) == CSpriteGen.EFFECT_NONE)
                {
                        ...
                        if (v == OBSTACLE_PLATFORM)
                        {
                            ...
                            if (toadd.colMode==CSpriteGen.CM_BOX)
                            {
                                rhFrame.colMask.fillRectangle(rc.left, rc.top, rc.right,
                                		Math.min((int)(rc.top + CColMask.HEIGHT_PLATFORM), (int)rc.bottom), CColMask.CM_PLATFORM);
                            }
                            else
                            {
                                mask = rhApp.imageBank.getImageFromHandle(img).getMask(CMask.GCMF_OBSTACLE, 0, 1.0, 1.0);
    ...
    public void callBackDrawRoutines()
    {
        ...
        {
            int index;
            for (index = 0; index < rh4BackDrawRoutines.size(); index++)
            {
                CBackDraw routine = rh4BackDrawRoutines.get(index);
...
//	rh2PushedEvents=0;
        ...
        if (rhApp.parentApp==null)
        {
            ...
            else
            {
              ...
              /* if (rhFrame.joystick==CJoystick.JOYSTICK_ACCELEROMETER)
                {
    ...
    public int f_GameLoop()
    {
        ...
        if(!MMFRuntime.OUYA)
        {
            ...
            {
                rhPlayer[0]=(byte)rhApp.joystick.joystick;
            }
    /*        if (rhApp.joystickAcc!=nil)
            {
                rhPlayer[0]=(rhPlayer[0]&0xF0)|(rhApp.joystickAcc.joystick);
            }*/
        ...
        for (n = 0; n < 4; n++)
        {
            b = (byte) (rhPlayer[n] & plMasks[rhNPlayers][n]);
            b &= rh2InputMask[n];
            rhPlayer[n] = b;
            b ^= rh2OldPlayer[n];
            rh2NewPlayer[n] = b;
...
//	rh4DroppedFlag=0;
    ...
    void joyTest()
    {
        ...
        int ctrlKeys[][] = rhApp.getCtrlKeys();
        ...
        if(!MMFRuntime.OUYA)
        {
            ...
            {
                if (rhApp.getKeyState(ctrlKeys[0][k]))
                {
                    rhPlayer[0] |= (byte) (1 << k);
                }
            }
    ...
    public void newHandle_Collisions(CObject pHo)
    {
        ...
        // ---------------------------------------------------------------------------
        if ((pHo.hoLimitFlags & CObjInfo.OILIMITFLAGS_QUICKCOL) != 0)
        {
            ArrayList<CObject> cnt = objectAllCol_IXY(pHo, pHo.roc.rcImage, pHo.roc.rcAngle, pHo.roc.rcScaleX, pHo.roc.rcScaleY, pHo.hoX, pHo.hoY);
            if (cnt != null)
            {
                int obj;
     ...
     */
    public ArrayList<CObject> objectAllCol_IXY(CObject pHo, short newImg, float newAngle, float newScaleX, float newScaleY, int newX, int newY)
    {
        ...
        if ((pHo.hoFlags & (CObject.HOF_REALSPRITE | CObject.HOF_OWNERDRAW)) != 0)
        {
            ...
            {
                list = spriteGen.spriteCol_TestSprite_All(pHo.roc.rcSprite, newImg, newX - rhWindowX, newY - rhWindowY, newAngle, newScaleX, newScaleY, 0);
            }
    ...
    public String get_EventExpressionStringLowercase (CParamExpression pExp)
    {
        ...
        return get_EventExpressionString (pExp);
    }
    ...
    public void setFrameRate(int value)
    {
        if (value >= 1 && value <= 1000)
        {
            ...
            app.refTime = System.currentTimeMillis() - (int) (((double) rhVBLOld * 1000.0f) / rhApp.gaFrameRate);
        }
    }
    ...
    public void reinitDisplay ()
    {
        ...
        if (rhNObjects != 0)
        {
            ...
            do
            {
                ...
                rhObjectList[count ++].reinitDisplay();
