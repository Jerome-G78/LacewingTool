...
//    public static final int LEF_RESERVED_2=0x2000;
    ...
    public boolean loadFullFrame(int index)
    {
		...
		evtProg.enumSounds((IEnum)app.musicBank, (IEnum)app.soundBank);
    ...
    public void updateSize()
    {
        ...
        {
            leEditWinWidth=Math.min((int)app.gaCxWin, leWidth);
            leEditWinHeight=Math.min((int)app.gaCyWin, leHeight);
        }
    }
...
//			    LoadOnCall(poi);
    ...
    public boolean bkdLevObjCol_TestSprite(CSprite pSpr, short newImg, int newX, int newY, float newAngle, float newScaleX, float newScaleY, int subHt, int nPlane)
    {
        ...
        if (!bSprColBox)
        {
            ...
            pSprMask = rhPtr.spriteGen.getSpriteMask(pSpr, (short) nImg, CMask.GCMF_OBSTACLE, newAngle, newScaleX, newScaleY);
            if (pSprMask == null)
            {
    ...
    boolean colMask_TestSprite(CSprite pSpr, int newImg, int newX, int newY, float newAngle, float newScaleX, float newScaleY, int subHt, int nPlane)
    {
        ...
        int x1 = newX;
        int y1 = newY;
        int nColMode = (int) rhPtr.spriteGen.colMode;
        int nWidth, nHeight;
        CRect rc = new CRect();
        ...
        if (nColMode != CSpriteGen.CM_BOX && (pSpr.sprFlags & CSprite.SF_COLBOX) == 0)
        {
            ...
            pMask = rhPtr.spriteGen.getSpriteMask(pSpr, (short) nImg, CMask.GCMF_OBSTACLE, newAngle, newScaleX, newScaleY);
            if (pMask == null)
            {
