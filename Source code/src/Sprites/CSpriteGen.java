...
public class CSpriteGen {
	...
	// -------------------------------------------------------------
	public CSprite addSprite(int xSpr, int ySpr, short iSpr, short wLayer,
			int nZOrder, int backSpr, int sFlags, CObject extraInfo) {
	...
	// ------------------------------------------------------;
	public CSprite addOwnerDrawSprite(int x1, int y1, int x2, int y2,
			short wLayer, int nZOrder, int backSpr, int sFlags,
			CObject extraInfo, IDrawable sprProc) {
	...
	public CSprite modifSprite(CSprite ptSpr, int xSpr, int ySpr, short iSpr) {
        ...
        // Comparison
        if (ptSpr.sprXnew != xSpr || ptSpr.sprYnew != ySpr
	...
	public CSprite modifSpriteEx(CSprite ptSpr, int xSpr, int ySpr, short iSpr,
			float fScaleX, float fScaleY, boolean bResample, float nAngle,
			boolean bAntiA) {
		// Pointer sur le sprite et son image
		do {
			if (ptSpr == null) {
			...
			// Plus tard: autoriser les valeurs n�gatives et faire ReverseX /
			// ReverseY
			if (fScaleX < 0.0f) {
				fScaleX = 0.0f;
			}
			...
			// Comparison
			if (bRecalcSurf != false || ptSpr.sprXnew != xSpr
					|| ptSpr.sprYnew != ySpr || ptSpr.sprImgNew != iSpr) {
				// Recalc surface?
				...
				// Modification par rapport � la version pr�c�dente,
				// le ReafSpr est appel� seulement s'il y a changement dans
				// les coordonn�es ou l'image

				// Update colliding sprites
				if ((ptSpr.sprFlags & CSprite.SF_HIDDEN) == 0
						&& (ptSpr.sprFlags & (CSprite.SF_INACTIF
								| CSprite.SF_REAF | CSprite.SF_DISABLED)) != 0) {
					ptSpr.sprFlags |= CSprite.SF_REAF;
				}
			}
		} while (false);
	...
	// ------------------------------------------;
	public CSprite modifSpriteEffect(CSprite ptSpr, int effect, int effectParam) {
	...
	// Modification des caracteristiques d'un sprite ;
	// --------------------------------------------------;
	public CSprite modifOwnerDrawSprite(CSprite ptSprModif, int x1, int y1,
			int x2, int y2) {
	...
	// Set sprite angle
	//
	public void setSpriteAngle(CSprite ptSpr, int nAngle, boolean bAntiA) {
		if (ptSpr != null) {
			boolean bOldAntiA = ((ptSpr.sprFlags & CSprite.SF_ROTATE_ANTIA) != 0);
			nAngle %= 360;
			if (nAngle < 0) {
        ...
        for (ptSpr1 = firstSprite; ptSpr1 != null; ptSpr1 = ptSpr1.objNext)
        {
            ...
            if (x1 < ptSpr1.sprX2 && x2 > ptSpr1.sprX1 && y1 < ptSpr1.sprY2 && y2 > ptSpr1.sprY1)
            {
                ...
                else
                {
                    ...
                    pMask2 = getSpriteMask(ptSpr1, (short) -1, nGetColMaskFlag, ptSpr1.sprAngle, (double) ptSpr1.sprScaleX, (double) ptSpr1.sprScaleY);
                    if (pMask2 != null)
                    {
            ...
            if (x1 < ptSpr1.sprX2 && x2 > ptSpr1.sprX1 && y1 < ptSpr1.sprY2 && y2 > ptSpr1.sprY1)
            {
                ...
                if ((ptSpr1.sprFlags & CSprite.SF_TOKILL) == 0)	// Securit�?
                {
                    ...
                    else
                    {
                        ...
                        // Test collision with second sprite
                        CMask pMask2 = getSpriteMask(ptSpr1, (short) -1, nGetColMaskFlag, ptSpr1.sprAngle, ptSpr1.sprScaleX, ptSpr1.sprScaleY);
                        if (pMask2 != null)
                        {
			...
			if (bEvenNoCol || (ptSpr.sprFlags & CSprite.SF_RAMBO) != 0) {
				if (xp <= ptSpr.sprX2 && xp + wp > ptSpr.sprX1
					...
					// happen
					{
						...
						// Fine collision mode, test bit image
						CMask pMask = getSpriteMask(ptSpr, (short) -1,
								nGetColMaskFlag, ptSpr.sprAngle, ptSpr.sprScaleX,
                                ptSpr.sprScaleY);
						if (pMask != null) {
							if (pMask.testRect(0, xp - ptSpr.sprX1, yp
									- ptSpr.sprY1, wp, hp)) {
								return ptSpr;
							}
						}
