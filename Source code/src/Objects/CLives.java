        ...
        switch (adCta.odDisplayType)
        {
            case 4:	    // CTA_ANIM:
                if (vInt != 0)
                {
                    ...
                    int lg = vInt * ifo.getWidth();
                    if (lg <= rsBoxCx)
                    {
                        hoImgWidth = (short) lg;
                        hoImgHeight = ifo.getHeight();
                    }
                    else
                    {
                        hoImgWidth = rsBoxCx;
                        hoImgHeight = ((rsBoxCx / ifo.getWidth()) + vInt - 1) * ifo.getHeight();
                    }
                    break;
                }
                hoImgWidth = hoImgHeight = 1;		// 0
            ...
            case 5:	    // CTA_TEXT:
                ...
                rTemp.copyRect (rc);
                textSurface.measureText (s, dtflags, font.getFontInfo(), rTemp);
        ...
        switch (adCta.odDisplayType)
        {
            case 4:	    // CTA_ANIM:
                if (vInt != 0)
                {
                    ...
                    for (y = y1; y < y2 && vInt > 0; y += ifo.getHeight())
                    {
                        for (x = x1; x < x2 && vInt > 0; x += ifo.getWidth(), vInt -= 1)
                        {
                            hoAdRunHeader.spriteGen.pasteSpriteEffect(adCta.frames[0], x, y, 0, effect, effectParam);
                        }
                    }
                }
                break;
